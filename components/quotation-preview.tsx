...
export function QuotationPreview({ data, settings }: QuotationPreviewProps) {
  const [showTerms, setShowTerms] = useState(true)
  const [showNotes, setShowNotes] = useState(true)
  const [showDiscount, setShowDiscount] = useState(true)
  const [showFinancialInfo, setShowFinancialInfo] = useState(true)
  const [showTotalValues, setShowTotalValues] = useState(true)
  ...
  const numberToWords = (num: number): string => {
    ...
    if (decimal > 0) {
      result = result.trim();
      result += ' and ' + convertLessThanThousand(decimal).trim() + 'Cents';
    }
  ...
  const handleDesktopDownload = async () => {
    ...
    try {
      // Add a 3-second delay for better UX feedback
      await new Promise(resolve => setTimeout(resolve, 500))
      
      await generatePDFFromElement(element, {
        fileName: `${data.quotationNumber || "draft"}.pdf`,
        scale: 2
      })
    } catch (error) {
          ...
          <div className="flex items-center gap-6 border-r pr-6">
            ...
            <label className="flex items-center gap-2 text-sm whitespace-nowrap">
              <input
                ...
                onChange={(e) => setShowTotalValues(e.target.checked)}
                className="rounded border-gray-300"
              />
              Show Total Amounts
            </label>
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4 md:gap-0">
            <div className="flex items-center gap-3 w-full md:w-auto">
              {settings?.logoUrl ? (
                ...
                <div className="w-10 h-10 md:w-12 md:h-12 bg-emerald-600 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-white font-bold text-base md:text-lg">{data.companyName.substring(0, 2).toUpperCase()}</span>
                </div>
              )}
              <div className="min-w-0 flex-1">
                <h1 className="text-xl md:text-2xl font-bold text-slate-800 leading-tight break-words">{data.companyName}</h1>
                <p className="text-slate-600 text-xs md:text-sm leading-tight break-words">{data.companyAddress}</p>
              </div>
            </div>
            <div className="text-left md:text-right w-full md:w-auto">
              <h2 className="text-2xl md:text-3xl font-bold text-slate-800">Quotation</h2>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-4 mb-6">
            <div className="space-y-0.5">
              <h3 className="font-semibold text-slate-800 mb-1 text-sm md:text-base">Quotation by</h3>
              <p className="text-slate-700 font-medium leading-tight text-sm md:text-base break-words">{data.companyName}</p>
              <p className="text-slate-600 text-xs md:text-sm leading-tight whitespace-pre-line break-words">{data.companyAddress}</p>
              <p className="text-slate-600 text-xs md:text-sm leading-tight break-words">{data.companyPhone}</p>
              <p className="text-slate-600 text-xs md:text-sm leading-tight break-words">{data.companyEmail}</p>
            </div>

            <div className="space-y-0.5">
              <h3 className="font-semibold text-slate-800 mb-1 text-sm md:text-base">Quotation to</h3>
              <p className="text-slate-700 font-medium leading-tight text-sm md:text-base break-words">{data.clientName || "Client Name"}</p>
              <p className="text-slate-600 text-xs md:text-sm leading-tight whitespace-pre-line break-words">{data.clientAddress || "Client Address"}</p>
            </div>

            <div>
              <div className="space-y-1">
                <div className="flex justify-between">
                  <span className="text-slate-600 text-xs md:text-sm">Quotation No:</span>
                  <span className="font-medium text-xs md:text-sm break-words">{data.quotationNumber}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-600 text-xs md:text-sm">Quotation Date:</span>
                  <span className="font-medium text-xs md:text-sm">{new Date(data.quotationDate).toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-600 text-xs md:text-sm">Due Date:</span>
                  <span className="font-medium text-xs md:text-sm">{new Date(data.dueDate).toLocaleDateString()}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <table className="w-full border-collapse border border-slate-300">
                <thead>
                  <tr className="bg-slate-100">
                    <th className="border border-slate-300 py-2 px-3 text-left font-semibold text-sm">Item #/Item description</th>
                    <th className="border border-slate-300 py-2 px-3 text-center font-semibold text-sm">Quantity</th>
                    <th className="border border-slate-300 py-2 px-3 text-center font-semibold text-sm">Rate</th>
                    {showDiscount && (
                      <th className="border border-slate-300 py-2 px-3 text-center font-semibold text-sm">Discount</th>
                    )}
                    <th className="border border-slate-300 py-2 px-3 text-center font-semibold text-sm">
                      Amount
                      {showDiscount && showTotalValues && <span className="text-xs font-normal block">(After Discount)</span>}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {data.items.map((item, index) => (
                    <tr key={item.id}>
                      <td className="border border-slate-300 p-3">
                        <div className="font-medium text-sm break-words">{item.description || `Item ${index + 1}`}</div>
                      </td>
                      <td className="border border-slate-300 p-3 text-center text-sm">{item.quantity}</td>
                      <td className="border border-slate-300 p-3 text-center text-sm whitespace-nowrap">Rs. {formatCurrency(item.rate)}</td>
                      {showDiscount && (
                        <td className="border border-slate-300 p-3 text-center text-sm">{item.discountType === 'amount' ? `Rs. ${formatCurrency(item.discount)}` : `${item.discount}%`}</td>
                      )}
                      <td className="border border-slate-300 p-3 text-center font-medium text-sm whitespace-nowrap">
                        Rs. {formatCurrency(showTotalValues ? (showDiscount ? calculateItemTotal(item) : item.quantity * item.rate) : item.quantity * item.rate)}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            <div className="space-y-4 md:space-y-6">
              ...
              {showTerms && (
                <div>
                  <h3 className="font-semibold text-slate-800 mb-2 md:mb-3 text-sm md:text-base">Terms and Conditions</h3>
                  <p className="text-slate-600 text-xs md:text-sm whitespace-pre-line break-words">{data.terms}</p>
                </div>
              )}
              ...
              {showNotes && (
                <div>
                  <h3 className="font-semibold text-slate-800 mb-2 md:mb-3 text-sm md:text-base">Additional Notes</h3>
                  <p className="text-slate-600 text-xs md:text-sm whitespace-pre-line break-words">{data.notes}</p>
                </div>
              )}
              ...
              {showTotalValues && (
                <>
                  <div className="border border-slate-300">
                    ...
                    <div className="flex justify-between p-2 md:p-3 bg-emerald-600 text-white">
                  ...
                  <div className="border border-slate-300 p-2 md:p-3">
                    <div className="font-medium mb-1 text-xs md:text-sm">Quotation Total In Words:</div>
                    <div className="text-xs md:text-sm text-slate-600 break-words">{numberToWords(showDiscount ? total : subtotal)} Sri Lankan Rupees Only</div>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="mt-8 md:mt-12 pt-3 md:pt-4 border-t border-slate-300 text-center">
            <p className="text-slate-600 text-xs md:text-sm break-words px-2">
              For any enquiries, email us on {data.companyEmail} or call us on {data.companyPhone}
            </p>
          </div>
        </div>
      </div>
